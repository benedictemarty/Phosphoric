...
- **Chunky 4bpp** : 160 px/rangée × 200, chacun 4 bits = index dans la **LUT palette NG 16 entrées** (RGB888, §5.1). 80 octets/rangée (quartet haut = pixel gauche). Chaque pixel chunky occupe 2 px framebuffer → **320 px** de large (`OCULA_EXTHIRES_W`). Rangées 200-223 = texte de statut standard (`$BB80`). Test visible : `NG_MODE=$05` + palette index 0 = magenta → framebuffer **320×224**, magenta dominant (zones à zéro) + autres couleurs des données = 16 couleurs.
