...
#include "utils/netutil.h"   /* parse_host_port (pur string, portable) */
...
/* ── TCP backpressure (v1.105.0) — no-op: tcp not built on Windows v1 ─ */

void serial_backend_tcp_set_rcvbuf(serial_backend_t* backend, int bytes)
{
    (void)backend; (void)bytes;   /* tcp backend is unavailable here */
}

/* ── unified transport factory (mirrors serial_backend.c) ───────────
 * Same dispatch as the POSIX build: loopback and file: are fully
 * functional; every other spec routes to a factory that returns NULL
 * with a clear "Windows v1" message. parse_host_port is pure string
 * parsing (utils/netutil.c), so tcp:/modem: still parse before the
 * unavailable message. */
